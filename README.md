...
`yt-dlp.exe` と `ffmpeg.exe` はライセンス対応を曖昧にしないため、リポジトリおよび標準配布物へ同梱しません。`setup-tools.ps1` が公式配布元から直接取得し、`MovieDownloader.exe` と同じフォルダへ配置します。
...
- URL 入力後の非同期メタデータ取得
- 実進捗、速度、残り時間、キャンセル
- 字幕、サムネイル埋め込み、WAV 変換、ログを詳細設定へ格納
